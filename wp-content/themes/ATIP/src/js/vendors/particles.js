	...
	constructor( tag_id, params ) {
			...
			switch ( p.shape ) {
				...
				case 'polygon':
					pJS.fn.vendors.drawShape(
						...
						p.x -
							radius /
								( pJS.particles.shape.polygon.nb_sides / 3.5 ),
						p.y - radius / ( 2.66 / 3.5 ),
						( radius * 2.66 ) /
							( pJS.particles.shape.polygon.nb_sides / 3 ),
				...
				case 'star':
					pJS.fn.vendors.drawShape(
						...
						p.x -
							( radius * 2 ) /
								( pJS.particles.shape.polygon.nb_sides / 4 ),
			...
			for ( var i = 0; i < pJS.particles.array.length; i++ ) {
				...
				if ( pJS.particles.opacity.anim.enable ) {
					if ( p.opacity_status == true ) {
						...
						if (
							p.opacity <= pJS.particles.opacity.anim.opacity_min
						)
				...
				/* events */
				if (
					isInArray( 'grab', pJS.interactivity.events.onhover.mode )
				) {
		...
		pJS.fn.modes.bubbleParticle = function ( p ) {
			...
			if (
						...
						if (
							...
							if (
								...
								var opacity =
									p.opacity -
									( pJS.particles.opacity.value -
										pJS.interactivity.modes.bubble
											.opacity ) *
										ratio;
								if (
									opacity < p.opacity &&
				...
				if ( pJS.tmp.bubble_clicking ) {
					...
					if (
						time_spent > pJS.interactivity.modes.bubble.duration
					) {
					...
					if (
						time_spent >
						pJS.interactivity.modes.bubble.duration * 2
					) {
		...
		pJS.fn.modes.repulseParticle = function ( p ) {
			if (
				...
				var normVec = {
					...
					repulseFactor = clamp(
						( 1 / repulseRadius ) *
							( -1 * Math.pow( dist_mouse / repulseRadius, 2 ) +
								1 ) *
							repulseRadius *
							velocity,
						0,
				...
				if ( pJS.particles.move.out_mode == 'bounce' ) {
					if (
						pos.x - p.radius > 0 &&
						pos.x + p.radius < pJS.canvas.w
					)
						p.x = pos.x;
					if (
						pos.y - p.radius > 0 &&
						pos.y + p.radius < pJS.canvas.h
					)
						p.y = pos.y;
				} else {
					p.x = pos.x;
			...
			if (
				...
				if ( dist_mouse <= pJS.interactivity.modes.grab.distance ) {
					var opacity_line =
						...
						dist_mouse /
							( 1 /
								pJS.interactivity.modes.grab.line_linked
									.opacity ) /
							pJS.interactivity.modes.grab.distance;
		...
		pJS.fn.vendors.densityAutoParticles = function () {
			if ( pJS.particles.number.density.enable ) {
				/* calc area */
				var area =
					( pJS.canvas.el.width * pJS.canvas.el.height ) / 1000;
				if ( pJS.tmp.retina ) {
				...
				var missing_particles =
					pJS.particles.array.length - nb_particles;
		...
		pJS.fn.vendors.drawShape = function (
			...
			var interiorAngle =
				Math.PI - ( Math.PI * interiorAngleDegrees ) / 180; // convert to radians
			c.save();
			c.beginPath();
...
export class particlesJS {
	...
	static load( tag_id, path_config_json, callback ) {
		/* load json config */
		var xhr = new XMLHttpRequest();
		...
		xhr.onreadystatechange = function ( data ) {
			if ( xhr.readyState == 4 ) {
				if ( xhr.status == 200 ) {
					...
					console.log(
						'Error pJS - XMLHttpRequest status: ' + xhr.status
					);
