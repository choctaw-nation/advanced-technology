...
$breadcrumb_links = array_map(
	function ( $name, $link ) {
		if ( is_null( $link ) ) {
			return '<li class="breadcrumb-item active fs-root" aria-current="page">' . esc_html( $name ) . '</li>';
		} else {
			return '<li class="breadcrumb-item"><a href="' . esc_url( $link ) . '" class="breadcrumb-item text-decoration-none fs-root">' . esc_html( $name ) . '</a></li>';
		}
	},
...
<nav aria-label="breadcrumb" class="mb-2">
	<ol class="breadcrumb m-0 column-gap-2 align-items-baseline" style="--bs-breadcrumb-divider: ''">
		<?php echo implode( '<i class="fas fa-caret-right w-auto" aria-hidden="true"></i>', $breadcrumb_links ); ?>
	</ol>
</nav>